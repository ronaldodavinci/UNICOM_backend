...
func (r *CommentRepository) Create(ctx context.Context, postID, userID bson.ObjectID, text string) (*model.Comment, error) {
	now := time.Now().UTC()
	doc := &model.Comment{
		ID:        bson.NewObjectID(),
		PostID:    postID,
...
func (r *CommentRepository) ListByPostNewestFirst(
	...
	cursorStr string,
	limit int64,
) (items []model.Comment, next *string, err error) {
...
func (r *CommentRepository) Update(ctx context.Context, commentID, userID bson.ObjectID, newText string, isRoot bool) (*model.Comment, error) {
	var c model.Comment
